 ...
 * not compile, and the two obvious workarounds are both bad: `!` says
 * nothing when the section is missing, and `?.` walks on and fails several
 * lines later on a value nobody can trace back.
 *
 * These readers take a third way. A missing section throws where it is
 * read, naming which section it was.
 *
 * They also accept a null document, because `emitDocumentWithDiagnostics`
 * returns null when the emitter wrote nothing, and a test reading a section
 * already expects a document.
 *
 ...
 *
 * Keys inside a section are not guarded: `noUncheckedIndexedAccess` is off,
 * so a missing key gives an `undefined` that the following assertion reports
 * on its own.
 */
function section<T>(
 ...
 * tag says, such as how two applications of `@asyncTag` merge, is not about
 * where the tag is written, so it reads through this.
 *
