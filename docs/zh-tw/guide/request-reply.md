...
operation 是本應用在某個 channel 上做的事。[`@send`](../reference/decorators#send) 標記本應用送出的 message。[`@receive`](../reference/decorators#receive) 標記本應用接收的 message。兩個動作都是本應用視角，不是 broker 視角。
...
回覆走不同 channel 時，用 [`@replyChannel`](../reference/decorators#replychannel)。引數是帶有該 channel 的 interface 或 namespace，不是 channel 的 id。
...
回覆位址只有在執行期才知道時，用 [`@replyAddress`](../reference/decorators#replyaddress)。傳送端把位址放進 message，回應端從那裡讀出來。

這種情況下 AsyncAPI 要求回覆 channel 的 address 是 `null`。所以那個 channel 要用 [`@dynamicChannel`](../reference/decorators#dynamicchannel) 宣告。
...
`reply` 不是描述 request/reply 的唯一途徑。一對 operation 也能表達同一個交換，而且耦合更鬆。一邊送出請求並接收回應，另一邊做相反的事。每個 message 上的 [`@correlationId`](../reference/decorators#correlationid) 讓消費端知道某個回應對應到哪個請求。
