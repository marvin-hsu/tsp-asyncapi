---
title: "範例"
description: "這個 repository 的 [`examples/`](https://github.com/marvin-hsu/tsp-asyncapi/tree/main/examples) 底下有十六個完整範例。每個目錄有 TypeSpec 原始碼、編譯時用的 `tspconfig.yam..."
---

# 範例

這個 repository 的 [`examples/`](https://github.com/marvin-hsu/tsp-asyncapi/tree/main/examples) 底下有十六個完整範例。每個目錄有 TypeSpec 原始碼、編譯時用的 `tspconfig.yaml`，以及 emitter 產生的 `asyncapi.yaml`。
...
| [Protobuf payload](https://github.com/marvin-hsu/tsp-asyncapi/tree/main/examples/16-protobuf-payloads)              | `protobuf` 預覽功能。同一個 package 的兩個 message 共用該 package 的 proto3 文字。 |
