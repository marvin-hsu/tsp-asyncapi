...
除了 `header-property-dropped` 之外，這個套件的每個診斷都是錯誤。錯誤會擋掉所有寫檔。所以一次編譯要嘛寫出你要的 schema，要嘛一個都不寫。那條警告指出 record 略過的屬性，檔案照樣會寫出。
...
| `tsp-avro/header-property-dropped` | 屬性帶著 `@AsyncAPI.header`，record 略過它。       |
