---
title: "繼承與多型"
description: "一則 message 常有好幾種形態，共用一部分屬性。本頁說明 extends 怎麼輸出成 allOf，以及 @discriminator 怎麼讓讀取端分辨型別。"
---

# 繼承與多型

一則 message 常有好幾種形態，彼此共用一部分屬性。`extends` 把共用的部分抽出來，
`@discriminator` 告訴讀取端該看哪個屬性分辨是哪一種。

兩者分開用。只想共用屬性的話，`extends` 單獨用就好。
...
## `@discriminator`：靠哪個屬性分辨

多個子型別出現在同一個位置時才需要它。讀取端拿到一筆資料，得自己判斷是哪一種，
`@discriminator` 就是告訴它看哪個屬性。
