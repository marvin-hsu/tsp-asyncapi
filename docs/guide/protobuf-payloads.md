...
Each payload is a proto3 text that stands on its own. It keeps the `syntax` line, the `package` line, its own message, and every declaration that message reaches through its fields. A declaration the message never reaches is left out, so a payload describes itself and nothing else.

`OrderPlaced` names `Money` in a field, so its payload carries both declarations. `OrderShipped` names nothing, so its payload carries itself alone. The payload's message is the one message nothing else in the text references. That is how a Protobuf reader finds the root, and the slice guarantees it: every other declaration is there because a reference pulled it in. Two models of one package are therefore two payloads. The wire formats differ, and one shared schema would claim that one type decodes both.
