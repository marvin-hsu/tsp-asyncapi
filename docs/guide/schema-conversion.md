...
The conversion layer is implemented and covered by the unit test suite, but it is **not yet wired into `tsp compile` output** — today's emitted document has an empty `components`. It gets connected when message payloads land (see the [roadmap](https://github.com/marvin-hsu/typespec-asyncapi#roadmap)). Use this page to design models now; their schemas will appear in `components.schemas` exactly as shown here.
:::
