...
A payload is not always a TypeSpec model. [`@rawPayload`](./decorators/messages#rawpayload) and [`@rawHeaders`](./decorators/messages#rawheaders) carry a schema the author wrote in another language. A [preview feature](./emitter-options#preview-features) can generate one from the source. The emitter never reads inside either kind.
