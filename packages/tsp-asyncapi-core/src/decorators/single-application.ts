 ...
 *
 * This module owns the concept, so every caller names this type for its own
 * code. A caller with a copy of the declaration would keep compiling after
 * this one narrows, because the two remain assignable.
 */
 ...
 * both stop it, but only one of them is a mistake. `repeat` is the same
 * statement running again, and nothing is reported for it. `rejected` is a
 * second statement, and it is reported.
 */
 ...
 *
 * A decorator that carries one value can keep only one of them, so a second
 * application is reported rather than allowed to overwrite it in silence.
 *
 * The guard records that the decorator ran, and never reads the value it
 * stored. Those are different questions. Answering the first with the
  ...
  return {
    claim(context: DecoratorContext, target: Type): ClaimResult {
      ...
      if (claimed !== undefined) {
        ...
        // again for every reopened `namespace` block and every file that
        // opens the namespace. Those runs count as one application, so the
        // repeat is not reported, though it is still told apart from the
        // first run: the first run already stored the value and reported
        // whatever was wrong with it, and a caller that ran its body again
        // would report each of those problems once per declaration. Two
        // distinct statements can never share a file and an offset, so a
