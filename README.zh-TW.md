...
| 計劃項目                               | 目前的狀態                                                                                                                                                   | 說明                                                                               |
| -------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------ | ---------------------------------------------------------------------------------- |
| 對官方 `@typespec/protobuf` 的一級支援 | `protobuf` 預覽功能會用官方 decorator 產生 payload。用 `preview-features: ["protobuf"]` 開啟。`Protobuf.Map` 與 `Extern` model 仍只會降級成 `type: object`。 | 這是預覽功能，選項與診斷可能在次版本變更。用法見文件站台的 Protobuf payload 指南。 |
