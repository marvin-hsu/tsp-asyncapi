...
[`examples/`](./examples/) 底下有十四個完整範例，每一個都附 TypeSpec 原始碼與 emitter 產生的 AsyncAPI 文件。本 library 實作的每個通訊協定，都至少出現在其中一個範例裡。
...
| `components` 的可重用區塊 | `components` 目前只放 `schemas`、`messages` 與 `securitySchemes`，其餘就地展開。因為其他欄位通常只有單一使用點，抽出去再 `$ref` 只是多一次跳轉而無法節省空間，這與官方避免過度使用 Components 的理念一致（見 [#9171](https://github.com/microsoft/typespec/issues/9171)）。                                                                                                                              | 若出現需要重複輸出同一段就地展開內容的場景，這個決定會改變。     |
...
| 多檔輸出與跨檔 `$ref`            | TypeSpec 在原始碼層級就已經提供了很好的檔案切割與模組管理能力，因此最終產物通常沒有切檔的需求。不過，等未來官方的 EFv2 架構發展成熟後，會考慮實作多檔輸出功能。                                                                                                                                                                                             |
