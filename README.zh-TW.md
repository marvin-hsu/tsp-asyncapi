...
| 缺什麼                    | 目前的行為                                                                   | 什麼情況會改變決定                                                                                             |
| ------------------------- | ---------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------- |
| `components` 的可重用區塊 | `components` 只放 `schemas`、`messages` 與 `securitySchemes`，其餘就地展開。 | 其他欄位都只有一個使用點，抽出去再 `$ref` 只是多一次跳轉。若文件重複輸出同一段就地展開的內容，這個決定會改變。 |
...
| 多個 `@service`                  | 一份 AsyncAPI 文件描述一個應用。要描述多個應用，就為每個應用開一個入口專案，共用 TypeSpec 原始碼。目前輸出第一個 service，其餘由 `multiple-services` 回報。 |
| 多檔輸出與跨檔 `$ref`            | 一次編譯產出一份文件，所有引用都指向這份文件的內部。沒有切檔的需求。                                                                                        |
